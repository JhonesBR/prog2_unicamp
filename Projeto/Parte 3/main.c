...
int main(){
	...
	loadCliente(cliente, &qnt_cliente, &qnt_max_cliente);
	loadLocacao(locacao, &qnt_locacao, &qnt_max_locacao);
	loadVeiculo(veiculo, &qnt_veiculo, &qnt_max_veiculo);
	
	// Menus e submenus
	clearScreen();
	for(;escolha1 != 4;){
		saveCliente(cliente, &qnt_cliente);
		saveLocacao(locacao, &qnt_locacao);
		saveVeiculo(veiculo, &qnt_veiculo);
		printf("1 - Clientes\n");
		printf("2 - Locação\n");
		printf("3 - Veículos\n");
		printf("4 - Sair\n");
		...
		switch(escolha1){
			...
			switch(escolha2){
				...
				case 3:
					excluirCliente(cliente, &qnt_cliente, &qnt_max_cliente, &qnt_cliente_excluido, veiculo, &qnt_veiculo);
