...
// Função para salvar os locacoes em binário
void saveLocacao(struct locacoes locacao[], int *qnt_locacao){
	FILE *data;
	int n_locacoes = *qnt_locacao;
	
	// Grava todos os registros da memória no arquivo
	data = fopen("locacoes.dat", "wb"); // Cria o arquivo para escrita
	if (data == NULL)
		printf("\nErro na criacao do arquivo. Não foi possível gravar os registros.");
	else {
		fwrite(&n_locacoes, sizeof(int), 1, data);
		fwrite(locacao, sizeof(struct locacoes), n_locacoes, data);
		fclose(data);
	}
	*qnt_locacao = n_locacoes;
}

// Função para carregar os locacoes do binário
void loadLocacao(struct locacoes locacao[], int *qnt_locacao, int *qnt_max_locacao){
	FILE *data;
	int n_locacoes;
	
	data = fopen("locacoes.dat", "rb"); // Abre o arquivo para leitura
	if (data != NULL){ // Já existe o arquivo
		fread(&n_locacoes, sizeof(int), 1, data);
		
		// Verifica se existe espaço e aloca caso não tenha
		if (*qnt_max_locacao - n_locacoes <= 10){
			reallocSizeLocacao(locacao, *qnt_max_locacao + 20);
			(*qnt_max_locacao) = (*qnt_max_locacao) + 20;
		}
		
		fread(locacao, sizeof(struct locacoes), n_locacoes, data);
		fclose(data);
	} else
		n_locacoes = 0;
	*qnt_locacao = n_locacoes;
